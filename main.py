...
class Button:
    def __init__(self, screen, text, x, y, color, enable):  # 构造函数
        ...
        self.text_color = (255, 38, 38)  # 按钮文本颜色
        self.enable = enable
...
class WhiteStartButton(Button):  # 开始按钮（选白棋）
    def __init__(self, screen, text, x, y):  # 构造函数
        super().__init__(screen, text, x, y, [(19, 237, 255), (16, 252, 126)], True)
...
class GiveupButton(Button):  # 投降按钮（任何模式都能用
    def __init__(self, screen, text, x, y):
        super().__init__(screen, text, x, y, [(19, 237, 255), (16, 252, 126)], False)
...
class Game:  # pygame类
    ...
    def showWinner(self):  # 输出胜者
        def showFont(screen, text, location_x, locaiton_y, height):
            ...
            font_image = font.render(text, True, (253, 2, 240), (96, 255, 255))  # 文本及背景颜色
